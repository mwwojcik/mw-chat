...
I was expecting to see a lot of thread names changing when broadcasting and subscribing to signal.
But it turned out that all operations were performed by the same thread.

image::img/flux-parallel-thread-pool-per-subscriber.png[widht=80%]

**It turns out that the thread pool works a bit differently. There is no thread switching within one pipeline (the picture above on the left), instead of one thread is dedicated to servicing one subscriber. It carries out all operations within one pipeline (the picture above on the right).
